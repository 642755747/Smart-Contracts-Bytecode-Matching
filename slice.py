...
def draw(now_address, now_bb, now_stack, top, graph, opc, s_cnt):
    ...
    #   print(in_stack)
     ...
     #   print(top)
        ...
        if(len(features) - len(deleted_nodes) > 1):
            my_slice = dict()
            my_slice["edges"] = edges
            my_slice["features"] = features

            store_path = "./labeled_dataset/" + func_name
            if(os.path.exists(store_path) == False):
                os.mkdir(store_path)
            with open(store_path +"/" + opt_name + ".json", "w") as f:
...
pkl_path = "/home/gemini/workspace/evmdis/gigahorse-toolchain-master/vuln-src-result"
...
for pkl_name in pkl_list:
    ...
    position = pkl_name.find("-")
    func_name = pkl_name[0:position]
    opt_name = pkl_name[position+1:len(pkl_name)-4]
    cnt = cnt + 1
    print(cnt)
    if(cnt < 885188):
       continue
  ...
  #  time.sleep(5)
    ...
    #    print("************")
        top = 0
        start_address = next(iter(i.nodes()))
        #print(start_address)
        start_asm = i.nodes().get(start_address)
        stack = list()
        for _ in range(1000):
            stack.append({"0":"1", "value": "0x1"})
            top = top + 1
    # stack.append({"0":"1", "value": "0x1"})
        
        dfs(start_address, start_asm['asm'], stack, top, i, 0, False)
